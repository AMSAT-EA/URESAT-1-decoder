...
void procesar(char * file_name) {
   ...
   char * desc [] = { "-", "pwr", "tmp", "status", "pwr_sts", "tmp_sts", "sun", "radiometer", "antenna", "extended power - ina219", "chess_move", "chess_board" };
   ...
   do {
	   ...
	   if (num_bytes == 0) {
           ...
           if (num_bytes == 0 && (type == 0 || type >= 13 || source != 7)) {

		printf("\n\nPacket type is not valid (0-9, A, B or C) or source is not URESAT-1 (7)\n");
		fclose(f);
		return;
